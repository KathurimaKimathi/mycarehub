...
type Update interface {
	...
	UpdateClientCaregiver(ctx context.Context, caregiverInput *dto.CaregiverInput) (bool, error)
}
...
func (db *PGInstance) UpdateClientCaregiver(ctx context.Context, caregiverInput *dto.CaregiverInput) (bool, error) {
	var (
		client Client
	)
	...
	if err != nil {
		tx.Rollback()
		return false, fmt.Errorf("failed to update caregiver: %v", err)
	}

	if err := tx.Commit().Error; err != nil {
		tx.Rollback()
		return false, fmt.Errorf("transaction commit to update client caregiver failed: %v", err)
	}
